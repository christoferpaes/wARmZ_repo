...
#include <Windows.h>  // Include Windows.h for WinMain

struct ASTNode {
    std::string type;
    std::vector<ASTNode*> children;
    std::unordered_map<std::string, std::string> attributes; // Added for attributes
...
const wchar_t* CLASS_NAME = L"MayhemWindowClass"; // Use wide-character string for class name
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    wc.lpszClassName = CLASS_NAME; // Use wide-character string for class name
    RegisterClassW(&wc);

    HWND hwnd = CreateWindowExW(
        0,
        CLASS_NAME, // Use wide-character string for class name
        L"Mayhem Unleashed",
        WS_OVERLAPPEDWINDOW,
